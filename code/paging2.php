...
$result4 = mysqli_query($con,"SELECT * FROM anafores WHERE katastash='μη-ενεργό' ORDER BY aID  DESC  LIMIT $arxh, 20 ");
...
while ($row = mysqli_fetch_array($result4)) { 
      	...
      	echo '<h4>'."<font color='red'>Αναφορά </font>".'</h4>';
				...
				echo $row['sxolia'];
        echo'<h4>'. "<font color='green'>Λύτης : </font>".'</h4>';
        echo $row['luths'];
...
$number2=0;
$result5 = mysqli_query($con,"SELECT * FROM anafores WHERE katastash='μη-ενεργό' ");
...
for ($i=1; $i<=$selides; $i++) { 
     echo "<a href='paging2.php?page=".$i."'>".$i."</a> "; 
}
?>
</div>
</body>
